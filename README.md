...
阅读规则和摘要由 hook 一次性提供，不另外添加插件 System Prompt。无需
在输入框点名或调用本插件；自定义 SYSTEM.md 也无需加入 plugin_sections。
...
尚未生成首份摘要时，只提供一小段显式记忆请求的写入指引，所以第一次
就可以提出“请记住”。有摘要时这段要求已包含在完整阅读规则中，不重复
注入。必须实际写入 note 才算保存，不能仅凭模型回复“记住了”判断。
...
更新插件保留本地记忆与个人配置。
正式兼容基线从 **1.0.0** 开始；后续兼容更新会保留记忆、用户配置和引用
状态，升级失败不会以清空数据的方式恢复。
