...
- Model wire requests must use the pinned Kimi low-level requesters in vendor/kimi-requester through the private model bridge. Do not start a coding agent or session to generate memory. Preserve native thinking/tool messages transiently; extraction accepts only one completed submit_memory_extraction tool call with exactly the two required string arguments. Never fall back to assistant text, code fences, reasoning or guessed JSON. See ADR 0011 for conservative strict/auto transport policy.
