...
- Model wire requests must use the pinned Kimi low-level requesters in vendor/kimi-requester through the private model bridge. Do not start a coding agent or session to generate memory. Preserve native thinking/tool messages transiently, but accept extraction JSON only from completed final text, never a reasoning channel or arbitrary substring.
