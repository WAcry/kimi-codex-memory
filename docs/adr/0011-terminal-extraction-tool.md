...
结果选择由 ADR 0012 替代：收集完整流并验证完成状态后，逆序选择最后
一个合法结果工具；没有合法调用时才从 final text 选择最大的完整且
符合 schema 的 JSON 对象。不读取 reasoning，不修补 JSON。其他
来源仍按原有故障隔离、重试和退避工作，不新增即时纠错模型循环。
