...
提取优先使用 ADR 0011 的结果 Tool，按 ADR 0012 选择最后一个合法
调用。仅当没有合法调用时，才扫描 native final text 中完整且符合
同一结果契约的 JSON 对象。通用 bridge 仍接受已有 json_mode，提取
请求不使用它。工具参数不移除围栏、不修补，正文候选也不修补 JSON。
